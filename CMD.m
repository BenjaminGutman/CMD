...
function I = CMD(Measured, Kernels, I, tol, MaxIter, MinIter, Lambda, sigmas, enableGPUflag)

    % normilize PSF
    for i=1:length(Measured)
        Kernels{i} = Kernels{i}./sum(Kernels{i}(:));
        if enableGPUflag
            Kernels{i} = gpuArray(Kernels{i}); 
        end
    end
    
    % start by computing P*M
    adjPM = AdjointOperator(Measured, Kernels, sigmas, enableGPUflag);  %%adding the images together 

    % Solve P*PI=P*M using conjugate gradients
    temp = SquareOperator(I, Kernels, Lambda, sigmas, enableGPUflag);
    r = adjPM - temp;
    p = r;
    k = 0;
    err = inf;
    ...
    while (err>tol) && (k<MaxIter) || (k<MinIter)
        PadjPp = SquareOperator(p, Kernels, Lambda, sigmas, enableGPUflag);
...
% Compute P* D^2 X
function Output = AdjointOperator(Images, Kernels, sigmas, enableGPUflag)
    N = length(Kernels);
    Output = conv2(Images{1}, zeros(size(Kernels{1}), 'single'), 'full');
    if enableGPUflag
...
% Compute P* D^2 PX
function Output = SquareOperator(I, Kernels, Lambda, sigmas, enableGPUflag)
    Output = AdjointOperator(DirectOperator(I, Kernels), Kernels, ...
        sigmas, enableGPUflag) + Lambda*I;
end
